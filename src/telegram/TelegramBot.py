    ...
    async def cmd_repos(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for repo in repos[:15]:
                stars = f"Sterne: {repo['stars']}" if repo.get('stars', 0) > 0 else ""
                lang = repo.get('language') or ''
                techs = ', '.join((repo.get('technologies') or [])[:4])
            ...
            await update.message.reply_text("Zeitueberschreitung beim Abrufen der Repos.")
        except Exception as e:
